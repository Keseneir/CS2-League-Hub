...
app.post("/api/teams", requireAuth, async (req, res) => {
  try {
    ...
    if (!name || !tag) {
      return res.status(400).json({ error: "Название и тег обязательны." });
    }
    if (tag.length > 8) {
      return res.status(400).json({ error: "Тег не может быть длиннее 8 символов." });
    }

    const existing = await Team.findOne({ $or: [{ name }, { tag: tag.toUpperCase() }] });
    if (existing) {
      return res.status(400).json({ error: "Команда с таким названием или тегом уже существует." });
    }
...
app.post("/api/applications", requireAuth, async (req, res) => {
  try {
    if (!req.user.teamId) {
      return res.status(400).json({ error: "Сначала создайте команду." });
    }
...
<head>
  <meta charset="UTF-8">
  <title>Заявки — CS2 League Hub Admin</title>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: 'Inter', sans-serif; background: #0b0f12; color: #aebbc7; padding: 40px 20px; }
    h1 { font-family: 'Montserrat', sans-serif; color: #e6b022; font-size: 28px; margin-bottom: 30px; text-transform: uppercase; }
    .badge { background: rgba(230,176,34,0.12); border: 1px solid rgba(230,176,34,0.3); color: #e6b022; padding: 4px 12px; border-radius: 4px; font-size: 12px; font-weight: 700; letter-spacing: 1px; }
    table { width: 100%; border-collapse: collapse; background: #12171d; border: 1px solid #1f252c; border-radius: 10px; overflow: hidden; }
    thead tr { background: #0e1318; border-bottom: 2px solid #e6b022; }
    th { padding: 14px 16px; text-align: left; font-family: 'Montserrat', sans-serif; font-size: 11px; text-transform: uppercase; letter-spacing: 1.5px; color: #aebbc7; }
    tbody tr { border-bottom: 1px solid #1f252c; }
    tbody tr:last-child { border-bottom: none; }
    tbody tr:hover { background: rgba(255,255,255,0.02); }
    td { padding: 14px 16px; font-size: 13px; vertical-align: middle; }
    .toast { position: fixed; bottom: 24px; right: 24px; background: #12171d; border: 1px solid #1f252c; border-radius: 8px; padding: 14px 20px; font-weight: 600; font-size: 14px; box-shadow: 0 8px 32px rgba(0,0,0,0.5); display: none; }
    .count { color: #5c6b7f; font-size: 14px; margin-bottom: 20px; }
  </style>
</head>
<body>
  ...
  <div style="overflow-x:auto;">
    <table>
      ...
      <tbody>${rows || '<tr><td colspan="8" style="text-align:center;padding:40px;color:#5c6b7f;">Заявок пока нет</td></tr>'}</tbody>
    </table>
  </div>
  <div class="toast" id="toast"></div>
  <script>
    async function updateStatus(id, status) {
      const res = await fetch('/admin/applications/' + id + '/status', {
        method: 'PATCH',
      ...
      toast.textContent = status === 'accepted' ? '✅ Заявка принята' : '❌ Заявка отклонена';
      toast.style.display = 'block';
      toast.style.color = status === 'accepted' ? '#4caf82' : '#e05c5c';
